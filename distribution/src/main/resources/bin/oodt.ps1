...
function Set-DeploymentEnvironment {
    ...
    $env:PROTEUS_URL = "http://localhost:$TomcatPort/proteus-services"
    $env:SOLR_URL = "http://localhost:$SolrPort/solr"
    $env:SOLR_DRAT_URL = "$($env:SOLR_URL)/drat"
}
...
function Expand-PropertyFile([string] $Source) {
    ...
    foreach ($name in @(
        'OODT_HOME', 'OODT_BASE', 'DRAT_HOME', 'FILEMGR_HOME',
        'WORKFLOW_HOME', 'RESMGR_HOME', 'PCS_HOME', 'FILEMGR_URL',
        'WORKFLOW_URL', 'RESMGR_URL', 'OPSUI_URL', 'PROTEUS_URL',
        'SOLR_URL', 'SOLR_DRAT_URL')) {
...
function Start-Oodt {
    ...
    if (Test-Path -LiteralPath $solr) {
        ...
        if ($LASTEXITCODE -ne 0 -or -not (Wait-Port $SolrPort $true 90)) {
            Save-State $state
